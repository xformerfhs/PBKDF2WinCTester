...
Note, that this is a wrong calculation as the salt is interpreted as an integer and the password is hashed in the ANSI encoding. Here is the correct version where the salt is interpreted as a byted array and the password is converted to UTF-8 before it is hashed:
